    ...
    <div class="modal-content-large max-h-[90vh] overflow-y-auto" @click.stop>
      ...
      <div class="flex flex-col sm:flex-row gap-2 pt-4 border-t border-gray-200">
        ...
        <button
          ...
          class="flex-1 bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 transition-colors flex items-center justify-center gap-2 font-medium text-sm"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              stroke-linecap="round"
...
import { generateInvoicePDF, type InvoiceData } from '@/services/invoiceGenerator'
...
const downloadInvoice = () => {
  ...
  // Calcular totais
  const totalCredits = weeklyTransactions.value
    .filter(t => t.type === 'credit')
    .reduce((sum, t) => sum + t.value, 0)

  const totalConsumptions = weeklyTransactions.value
    .filter(t => t.type === 'consumption')
    .reduce((sum, t) => sum + t.value, 0)

  // Preparar dados para o PDF
  const invoiceData: InvoiceData = {
    student: store.currentStudent,
    transactions: weeklyTransactions.value,
    weekDescription: selectedWeekDescription.value,
    totalCredits,
    totalConsumptions,
    currentBalance: store.currentStudent.balance
  }
