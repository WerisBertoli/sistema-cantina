<template>
  <div class="modal-overlay" @click="store.closeModal()">
    <div class="modal-content-large max-h-[90vh] overflow-y-auto" @click.stop>
      <div class="flex justify-between items-center mb-3 sm:mb-4 bg-white dark:bg-gray-800 pb-2 sm:pb-3 border-b dark:border-gray-600">
        <h2 class="text-lg sm:text-xl font-semibold text-gray-800 dark:text-white">📊 Histórico Semanal</h2>
      ...
      <div class="mb-3 sm:mb-4 p-2 sm:p-3 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900 dark:to-indigo-900 rounded-lg border border-blue-100 dark:border-blue-800">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
          <div>
            <p class="text-xs text-gray-600 dark:text-gray-300 mb-1">Aluno:</p>
            <p class="font-semibold text-sm sm:text-base text-gray-800 dark:text-white truncate">{{ store.currentStudent?.name }}</p>
            <p class="text-xs text-gray-600 dark:text-gray-300 mt-1">Responsável:</p>
            <p class="font-medium text-xs sm:text-sm text-gray-700 dark:text-gray-200 truncate">{{ store.currentStudent?.parentName || 'responsável' }}</p>
          </div>
          <div class="text-left sm:text-right">
            <p class="text-xs text-gray-600 dark:text-gray-300 mb-1">Saldo atual:</p>
            <p class="font-bold text-base sm:text-lg" :class="getBalanceClass(store.currentStudent?.balance || 0)">
      ...
      <div class="mb-3 sm:mb-4 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
        <div class="bg-green-50 dark:bg-green-900 p-3 rounded-lg border border-green-200 dark:border-green-800">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="font-bold text-base text-green-600">{{ store.formatCurrency(totalCredits) }}</p>
            </div>
          </div>
        </div>
        
        <div class="bg-red-50 dark:bg-red-900 p-3 rounded-lg border border-red-200 dark:border-red-800">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="font-bold text-base text-red-600">{{ store.formatCurrency(Math.abs(totalConsumptions)) }}</p>
            </div>
          </div>
        </div>
      ...
      <!-- Lista de Transações -->
      <div class="mb-4">
        <h3 class="text-base font-semibold mb-3 text-gray-800 dark:text-white">📝 Movimentações dos últimos 7 dias</h3>
        
        <div v-if="weeklyTransactions.length === 0" class="text-center py-6 text-gray-500 dark:text-gray-400">
        ...
        <div v-else class="space-y-2">
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center">
                <div class="p-1.5 rounded-lg mr-2" :class="getTransactionIconClass(transaction.type)">
                  <svg class="w-4 h-4" :class="getTransactionIconColor(transaction.type)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path v-if="transaction.type === 'credit'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                    <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
                  </svg>
                </div>
                <div>
              ...
              <div class="text-right">
                <p class="font-bold text-base" :class="getTransactionValueColor(transaction.type)">
                  {{ transaction.type === 'credit' ? '+' : '-' }}{{ store.formatCurrency(Math.abs(transaction.value)) }}
                </p>
            ...
            <div v-if="transaction.type === 'consumption' && transaction.items" class="mt-2 pl-8">
              ...
              <div class="space-y-0.5">
                <div v-for="item in transaction.items" :key="item.productId" class="flex justify-between text-xs">
